...
function findDNS(records,target) {
        ...
        let low = 0;
        let high = recordsecords.length -1; 
    while (low <= high) {
        let mid = Math.floor((low + high) / 2); //Math.floor makes it so that it rounds down Math.ceiling mkaes it so that it rounds up (hence floor and ceiling)
